  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title h6 text-xs-center" id="myModalLabel"><i class="fa fa-check-square-o"></i>{l s='Product successfully added to your shopping cart' d='Shop.Theme.Checkout'}</h4>
      </div>
      <div class="modal-body">
        <div class="row">
          <div class="col-sm-6 divide-right">
            <div class="row">
              <div class="col-sm-6">
                <img class="product-image img-responsive" src="{$product.cover.medium.url}" alt="{$product.cover.legend}" title="{$product.cover.legend}" itemprop="image">
              </div>
              <div class="col-sm-6">
                <h6 class="h6 product-name">{$product.name}</h6>
                <p><strong>{l s='Price :' d='Shop.Theme.Checkout'}</strong>&nbsp;{$product.price}</p>
                {hook h='displayProductPriceBlock' product=$product type="unit_price"}
                {foreach from=$product.attributes item="property_value" key="property"}
                  <p><strong>{$property}&nbsp;:</strong>&nbsp;{$property_value}</p>
                {/foreach}
      ...
      <div class="modal-footer">
          <div class="cart-content-btn">
            ...
            <a href="{$cart_url}" class="btn btn-primary"><i class="fa fa-check-square-o"></i>{l s='proceed to checkout' d='Shop.Theme.Actions'}</a>
          </div>
      </div>
    </div>
  </div>
