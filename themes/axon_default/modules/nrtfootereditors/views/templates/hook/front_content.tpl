...
{if isset($node.row_s.bgw ) && $node.row_s.bgw == 1}<div class="container">{/if}
	...
	<div class="row">		
	{elseif $node.type==2}
		<div  class="{if $node.width_p==13}hidden-xs{else}col-xs-{$node.width_p}{/if} {if $node.width_t==13}hidden-sm{else}col-sm-{$node.width_t}{/if} {if $node.width_d==13}hidden-md hidden-lg{else}col-md-{$node.width_d}{/if} {if $node.contentType==0}empty-column{/if} {if $node.contentType==6}banner-column{/if}" >
				...
				{if isset($node.children) && $node.children|@count > 0}
					{foreach from=$node.children item=child name=categoryTreeBranch}
						...
						{if $smarty.foreach.categoryTreeBranch.first}  <ul class="nav nav-tabs">{/if} 
							<li class="nav-item"><a href="#content-tab-{$child.elementId}"  {if isset($child.tabtitle)}title="{$child.tabtitle}"{/if} data-toggle="tab">{if isset($child.tabtitle)}{$child.tabtitle}{else}{l s='Set tab title'}{/if}</a></li>
