...
<h4 class="title_block title_font">
	<a class="title_text" href="{$link->getModuleLink('blockwishlist', 'mywishlist', array(), true)|addslashes}" title="{l s='My wishlists'}" rel="nofollow">{l s='WISHLIST'}</a>
</h4>
	<div class="block_content">
		...
		{if $wishlist_products}
			...
			{foreach from=$wishlist_products item=product name=i}
				<dt class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
					...
					<a class="ajax_cart_block_remove_link" href="javascript:;" onclick="javascript:WishlistCart('wishlist_block_list', 'delete', '{$product.id_product}', {$product.id_product_attribute}, '0', '{if isset($token)}{$token}{/if}');" title="{l s='remove this product from my wishlist'}" rel="nofollow"><i class="fa fa-times-circle"></i></a>
				</dt>
		...
		{/if}
			<a class="btn title_font" href="{$link->getModuleLink('blockwishlist', 'mywishlist', array(), true)|addslashes}" title="{l s='My wishlists'}" rel="nofollow">{l s='MY WISHLISTS'}</a>
		</p>
	</div>
</div>
