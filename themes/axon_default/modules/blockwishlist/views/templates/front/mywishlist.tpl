...
<header class="page-header">
	...
	{if $id_customer|intval neq 0}
		...
		<form method="post" class="std" id="form_wishlist">
					...
					<input type="text" id="name" name="name" class="inputTxt form-control" value="{if isset($smarty.post.name) and $errors|@count > 0}{$smarty.post.name|escape:'html':'UTF-8'}{/if}" placeholder="{l s='Name'}" />
					<input type="submit" name="submitWishlist" id="submitWishlist" value="{l s='Save'}" class="btn" />
		</form>
		{if $wishlists}
			...
			<table class="std">
				<thead>
					<tr>
						...
						<th class="item mywishlist_second text-center">{l s='Default'}</th>
						<th class="last_item mywishlist_first text-center">{l s='Delete'}</th>
					</tr>
				</thead>
				<tbody>
				{section name=i loop=$wishlists}
					<tr id="wishlist_{$wishlists[i].id_wishlist|intval}">
						...
						<td><a href="javascript:;" onclick="javascript:WishlistManage('block-order-detail', '{$wishlists[i].id_wishlist|intval}');">{l s='View'}</a></td>
						<td class="wishlist_default text-center">
						...
						<td class="wishlist_delete">
							<a href="javascript:;"onclick="return (WishlistDelete('wishlist_{$wishlists[i].id_wishlist|intval}', '{$wishlists[i].id_wishlist|intval}', '{l s='Do you really want to delete this wishlist ?' js=1}'));"><i class="fa fa-trash"></i></a>
						</td>
					</tr>
				{/section}
				</tbody>
